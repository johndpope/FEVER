...
def doc_ranker(claim, title2wordlist):
    ...
    return heapq.nlargest(5, title2score, key=title2score.get), entity_list
...
def generate_dev_eval_doc_ranker():
    ...
    for line in readfile:
        if line.strip().find('NOT ENOUGH INFO') <0: #'SUPPORTS or REFUTES
            ...
            all_evi_list = line2dict.get('all_evidence')
            gold_doc_list = [tup[2] for tup in all_evi_list]
            ...
            if gold_doc_set.issubset(set(doc_list)):
                doc_recall_at_5+=1.0
            if gold_doc_set.issubset(set(doc_list[:-1])):
                doc_recall_at_4+=1.0
            if gold_doc_set.issubset(set(doc_list[:-2])):
                doc_recall_at_3+=1.0
            if gold_doc_set.issubset(set(doc_list[:-3])):
                doc_recall_at_2+=1.0
            if gold_doc_set.issubset(set(doc_list[:-4])):
                doc_recall_at_1+=1.0
            size+=1

            if size % 10 == 0:
                print  'size: ', size, ', recall_mean:', doc_recall_at_1/size,doc_recall_at_2/size,doc_recall_at_3/size,doc_recall_at_4/size,doc_recall_at_5/size
    writefile.close()
    readfile.close()
...
def compute_f1_two_list_names(pred_names, gold_names):
    ...
    return 2.0*recall*precision/(1e-8+recall+precision)
