...
def evaluate_lenet5(learning_rate=0.02, n_epochs=100, emb_size=300, batch_size=50, filter_size=[3], sent_len=40, claim_len=20, cand_size=10,hidden_size=[300,300], max_pred_pick=5):
    ...
    test_model = theano.function([sents_ids,sents_mask,sents_labels, claim_ids,claim_mask, joint_labels], [inter_matrix,test_layer_LR.errors(joint_labels)], allow_input_downcast=True, on_unused_input='ignore')
    ...
    while epoch < n_epochs:
        ...
        for joint_batch_id in joint_train_batch_start: #for each batch
            ...
            if iter%10==0:
                ...
                sents_ids,sents_mask,joint_sents_labels, claim_ids,claim_mask, joint_labels
                '''
                f1_sum=0.0
                error_sum = 0.0
                for test_batch_id in test_batch_start: # for each test batch
                    batch_prob, error_i=test_model(
                ...
                acc = 1.0 - error_sum/(len(test_batch_start)+len(test_3th_batch_start))
                if test_f1 > max_test_f1:
                    max_test_f1=test_f1
                if acc > max_acc:
                    max_acc = acc
                print '\t\tcurrent: ', test_f1,acc,' ; ','\t\t\t\t\tmax: ', max_test_f1, max_acc
